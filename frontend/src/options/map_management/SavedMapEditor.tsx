...
const SavedMapEditor = (): React.ReactElement => {
    ...
    return <SavedMapEditorForMap key={mapId} mapId={mapId}/>;
};

const SavedMapEditorForMap = ({mapId}: {mapId?: string}): React.ReactElement => {
    ...
    const mapIsCurrent = map?.current === true;
    const editable = mapIsCurrent && currentMapData?.metaData.dreameRismMapId === Number(mapId);
    const roomEditingDisabled = !robotStatus || !["idle", "docked"].includes(robotStatus.value);
    const [
    ...
    if (!editable) {
        return <PaperContainer><Typography color="error">The loaded map does not match this saved map. Wait for the robot to load the map before editing.</Typography></PaperContainer>;
    }
                                        ...
                                        <React.Fragment key={segment.id}>
                                            ...
                                            <ListItem sx={{display: "block", px: 0}}>
                                                <Box sx={{display: "flex", alignItems: "center", gap: 1, mb: 1}}>
                                                    ...
                                                    <IconButton size="small" disabled={index === 0 || orderSaving || visibilitySaving || roomEditingDisabled} onClick={() => offsetSegment(index, -1)}>
                                                        <ExpandLess/>
                                                    </IconButton>
                                                    <IconButton size="small" disabled={index === segments.length - 1 || orderSaving || visibilitySaving || roomEditingDisabled} onClick={() => offsetSegment(index, 1)}>
                                                        <ExpandMore/>
                                                    </IconButton>
                                                    {segment.visibility !== undefined && (
                                                        <IconButton
                                                            size="small"
                                                            disabled={visibilitySaving || orderSaving || roomEditingDisabled}
                                                ...
                                                <RoomPreferenceControls
                                                    segment={segment}
                                                    disabled={roomEditingDisabled || orderSaving || visibilitySaving}
                                                    mapId={mapId}
