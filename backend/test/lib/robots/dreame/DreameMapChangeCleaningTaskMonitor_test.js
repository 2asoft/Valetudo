...
describe("DreameMapChangeCleaningTaskMonitor", function() {
    ...
    it("should not replace a map-relative task while its map-change abort is pending", async function() {
        const fixture = makeMonitor();
        ...
        await assert.rejects(
            fixture.monitor.startMapRelativeCleanup({
                taskType: "zone",
                start: async () => {
                    fixture.calls.push("start_zone");
                }
        ...
        assert.equal(fixture.calls.includes("start_zone"), false);
    ...
    it("should reject a map-relative task while full-clean recovery STOP is in flight", async function() {
        ...
        await assert.rejects(fixture.monitor.startMapRelativeCleanup({
            taskType: "segment",
            start: async () => {
        ...
        assert.equal(fixture.calls.filter(call => call === "stop").length, 1);
        assert.equal(fixture.calls.filter(call => call === "segment_start").length, 0);
        assert.equal(fixture.monitor.getOperation().kind, "full");
        assert.equal(fixture.monitor.getOperation().mapId, 68);
    });

    it("should reject a map-relative task while full-clean recovery START is in flight", async function() {
    ...
    it("should not replace full-clean recovery before its STOP is dispatched", async function() {
        const fixture = makeMonitor();
        await startFullAndMarkActive(fixture);
